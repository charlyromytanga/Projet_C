...
/*Etape 0 : Remplissage des matrices de tous les nouveaux automates*/
automate remplissage(automate b, int val){
    /* Déclaration de variables locales*/
    int   *** m = b.matrice;
    int nb_Etats = b.nb_Etats, nb_Symboles = b.nb_Symboles;

    /*Instructions*/
    /*Allocation de mémoire pour la matrice du nouvel automate cré*/
    m = malloc(nb_Etats*sizeof(int **));

    /*Test 1 d'allocation réussi*/
    if(m == NULL){
        printf("L'allocation de mémoire a échoué!\n");
        exit(1);
    }
    ...
    for(int i=0; i<nb_Etats; i++){
        ...
        m[i] = malloc(nb_Symboles*sizeof(int *));
        ...
        for(int j=0; j<nb_Symboles; j++){
            ...
            m[i][j] =  malloc(nb_Etats*sizeof(int));

            /*Test 3 allocation de mémoire réussi*/
            if(m[i][j] == NULL){
    ...
    /*La fonction retourne une autre matrice*/
    return b;
...
automate creation_arc(automate a){
    ...
    /*L'automate passé en argument est l'automate complet qui a déjà subit tous les traitmement*/
    /*saut celui de raccorder tous les états hormis de l'état initial vers l'état phi par les transitions non utilisée */
    
    /*Déclaration de variables locales*/
    // L'automate qu'on va retourner, qui sera la version complette de l'automate non complet
    automate arc_en_plus;

    int ***m_arc_en_plus = arc_en_plus.matrice;
    /*Le nouvel automate aura les même caractéristique que l'automate passé en argument*/
    int nb_Etats = a.nb_Etats, nb_Symboles = a.nb_Symboles;

    /*La valeur traduisant la destination vers le nouvel état final de l'automate complet*/
    int val_phi = 1;  
    ...
    /*Allocation de mémoire pour la matrice du nouvel automate cré*/
    m_arc_en_plus = malloc(nb_Etats*sizeof(int **));

    /*Test 1 d'allocation réussi*/
    if(m_arc_en_plus == NULL){
        printf("L'allocation de mémoire a échoué!\n");
        exit(1);
    }
    ...
    for(int i=0; i<nb_Etats; i++){
        ...
        m_arc_en_plus[i] = malloc(nb_Symboles*sizeof(int *));

        /*Test 1 d'allocation réussi*/
        if(m_arc_en_plus[i] == NULL){
            printf("L'allocation de mémoire a échoué!\n");
            exit(1);
        }
        for(int j=0; j<nb_Symboles; j++){

            m_arc_en_plus[i][j] =  malloc(nb_Etats*sizeof(int));

            /*Test 3 allocation de mémoire réussi*/
            if(m_arc_en_plus[i][j] == NULL){
            ...
            for(int k=0; k<nb_Etats; k++){
                ...
                /*Dans l'ordre*/
                /*La création d'arc se traduit par m_ar_en_plus[i][j][k]=1*/

                /*Test si un pour un symbole, un arc a déjà été cré */
                /*Le états parcourus seront tout sauf l'état initial (i!=0)*/
                if(i != 0 & m_arc_en_plus[i][j] !=0){
                    /*Alors l'arc a déjà été cré*/
                    printf("l'arc : a_%d a déjà été cré", j);
                }else{
                    /*Création de l'arc en question*/
                    printf("Création de l'arc : a_%d", j);
                    m_arc_en_plus[i][j][k]=1;
                }
            }
        }
    }
...
/*copie des n première états de destinations de l'automate non complet*/
automate copie_automate (automate origine, automate copie, automate tmp){
    /*Déclaration de variables*/
    int ***m_o, ***m_c, ***m_tmp;
    m_o = origine.matrice, m_c = copie.matrice, m_tmp = tmp.matrice;

    int nb_Etats = origine.nb_Etats, nb_Symboles = origine.nb_Symboles;


    /*Instructions*/
    /*Test si les matrices ont la même taille avant copie*/
    if(nb_Etats != copie.nb_Etats & nb_Etats !=tmp.nb_Etats){
        printf(" Copie impossible taille incompatiblee\n");
        exit(1);
    }
    /*zone de copie*/
    for(int i=0; i<nb_Etats; i++){
        ...
        for(int j=0; j<nb_Symboles; j++){
            ...
            for(int k=0; k<nb_Etats; k++){
                ...
                m_tmp[i][j][k] = m_o[i][j][k];

                m_c[i][j][k] = m_tmp[i][j][k];
