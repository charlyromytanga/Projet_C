...
/* Mettre une valeur dans un tableau_1D*/
int *add_val_tab_1D(automate a, int *tab_in, int dim_1D, int val, int position){

    // Déclaration de variables locales
    int *tab_out = allocation_tab_1D(dim_1D);
    ...
    for(int i=0; i<dim_1D; i++){
        ...
        tab_out[position]=val;
    }

    // La fonction retourne : Tab_1D
    return tab_out;
}



/* Mettre une valeur dans un tableau_2D*/
void **add_val_tan_2D(automate a, int **tab_in, int dim1, int dim2, int val, int positon){

    // Déclaration de variables locales
    int ** tab_out=allocation_tab_2D(dim1, dim2);
    ...
    for(int i=0; i<dim1; i++){
        ...
        for(int j=0; j<dim2; j++){
            ...
            tab_out[i][positon]=val;
            
        }   
    }


    // La fonction retourne : Tab_2D
    return tab_out;
}
...
/* La fonction qui réaliste le produit de deux automates*/
automate a_multiply_b(automate a, automate b){

    // Déclaration de variavles locales
    automate a_b;

    // Instructions
    /* L'état initial a_b de la forme : s_0 = {{0},{0, 0}}*/
    int **s_0 = allocation_tab_1D(2);

    for(int i=0; i<2; i++){

        s_0[1]=allocation_tab_1D(2);
    }

    /* L'état courtant s_i aura même forme mais init avec des zéro*/
    int **s_i= allocation_tab_1D(2);
    for(int i=0; i<2; i++){

        s_0[1]=allocation_tab_1D(2);
    }

    /* Tenu de la mise à jour des tables de l'automate a_b*/

    /*Tab états finaux */
    int ***tab_finaux_a_b = allocation_tab_1D(a.nb_Etats+b.nb_Etats);
    for(int i=0; i<(a.nb_Etats+b.nb_Etats); i++){

        tab_finaux_a_b[i] = allocation_tab_1D(2);
        for(int j=0; j<2; j++){

            tab_finaux_a_b[i][1] = allocation_tab_1D(2);
        }
    }

    /*Tab symbole avec une particularité,  les symboles du second automate ont*/
    /*un Zéro en plus devant les indices de postions, même avec cette particularité on considère */
    /* A indice égale les symboles sont les mêmes ex : a_1 pour l'automate a et a_0_1 pour b renvoient : a_0*/
    
    /* Avec un petit compteur lors des ajouts pour connaitre la taille et ont peut mettre des -1 à la place */
    int *tab_symbole_a_b = allocation_tab_1D(a.nb_Symboles+b.nb_Symboles);

    /* Tab des états de l'automate produit avec un petit compteur pour connaitre le nombre d'états*/
    int ***tab_etats_a_b = allocation_tab_1D(a.nb_Etats+b.nb_Etats);

    for(int i=0; i<(a.nb_Etats+b.nb_Etats); i++){

        tab_etats_a_b[i] = allocation_tab_1D(2);
        for(int j=0; j<2; j++){

            tab_etats_a_b[i][1] = allocation_tab_1D(2);
        }
    }

    /* Tab des transitions sera fait un peu plus tard*/

    /* Test de conditions pour le parcours de la boucles principale : min des nombdres de symboles*/

    /*Voici la grande boucle*/
    for(int i_symbol_a=0; i_symbol_a < a.nb_Symboles; i_symbol_a++){

        
        for(int j_state=0; j_state<a.nb_Etats; j_state++){

            /* Le tableau qui récupère les valeurs des tests si par les états respectifs*/
            /* de a et b, et par application du symbole i_symbol_a, des états ont été attients*/

            /* Le outils : */
            int state_res_a=0;
            int *state_res_b = allocation_tab_1D(2);

            int final_state_a=0, final_state_b=0;

            for(int k_state_a=0; k_state_a<a.nb_Etats; k_state_a++){
                
                
                state_res_a = a.matrice[i_symbol_a][j_state][k_state_a];

                state_res_b = b.matrice[i_symbol_a][j_state][k_state_a];

                /*Test sur les valeurs retournée qui doivent être toutes égales à UN*/
                if(state_res_a==1 || state_res_b==1){

                    /* On met à jour l'état courant de l'automate produit a_b*/
                    for(int i1=0; i1<2; i1++){
                        ...
                        for(int i2=0; i2<2; i2++){
                            ...
                            for(int j=0; j<2; j++){
                                ...
                                s_i[1][0]=state_res_b[0];
                                s_i[1][0]=state_res_b[1];

                            }
                        }
                    }

                    /* On test si les états cilbes sont des états finaux auquel cas on les ajoutes au tableau*/
                    /* des états finaux de l'automate produit a_b*/
                    final_state_a = est_un_etat_final(a, state_res_a);
                    final_state_b = est_un_etat_final(b, state_res_b);

                    if( (final_state_a==1) || (final_state_b==1)){

                        /* Voici ce tableau des états finaux qu'on met à jour*/
                        for(int i=0; i<(a.nb_Etats+b.nb_Etats); i++){
                            ...
                            for(int j=0; j<2; j++){
                                ...
                                for(int k=0; k<2; k++){
                                    ...
                                    if((final_state_a==1)){
                                        ...
                                        /*Si c'est issu de a*/
                                        tab_finaux_a_b[i][j]=state_res_a;

                                    }else if ((final_state_b==1))
                                    {
                                        /* Si c'est issu de b */
                                        tab_finaux_a_b[i][j][0]=state_res_b[0];
                                        tab_finaux_a_b[i][j][1]=state_res_b[1];
                        ...
                        /* Voici ce tableau des états de l'automate produit qu'on met à jour*/
                        for(int i=0; i<(a.nb_Etats+b.nb_Etats); i++){
                            ...
                            for(int j=0; j<2; j++){
                                ...
                                for(int k=0; k<2; k++){
                                    ...
                                    if((final_state_a==1)){
                                        ...
                                        /*Si c'est issu de a*/
                                        tab_etats_a_b[i][j]=state_res_a;

                                    }else if ((final_state_b==1))
                                    {
                                        /* Si c'est issu de b */
                                        tab_etats_a_b[i][j][0]=state_res_b[0];
                                        tab_etats_a_b[i][j][1]=state_res_b[1];
                    ...
                    /* On met à jour la table des symboles de l'automate produit*/
                    /* Voici ce tableau de symboles*/

                    for(int i=0; i<(a.nb_Symboles+b.nb_Symboles); i++){

                        tab_symbole_a_b[i]= add_val_tab_1D(a, tab_symbole_a_b, (a.nb_Symboles+b.nb_Symboles), j_state, i);
                    }
    ...
    /* On compte les nombres de symboles et d'états afin d'attribuer toutes les proprietés à l'autoamte produit*/

    
    /* On lie tout à l'automate produit*/

    // La la fonction retourne : un automate
    return a_b;
}
